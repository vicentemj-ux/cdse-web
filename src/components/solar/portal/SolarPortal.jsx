...
function printQuote(quote) {
  ...
  <section class="page"><div class="topline"><div class="brand"><span class="brand-mark">C</span> CDSE SOLAR</div><div class="date">COTIZACION<strong>${esc(quote.folio)}</strong><span>${esc(date)}</span></div></div>
    <div class="cover"><div><div class="eyebrow">Propuesta solar personalizada</div><h1>Tu energia.<br><span>Tu ahorro.</span></h1><p>Una propuesta clara para producir tu propia energia con el respaldo local de CDSE.</p><div class="client-card"><span class="label">Preparada para</span><span class="value">${esc(lead)}</span><p style="margin:5px 0 0;color:#53677b">Los Mochis y alrededores</p></div></div><div class="cover-image"></div></div>
    <div class="investment"><div><span class="label">Inversion del proyecto</span><strong>${esc(money.format(Number(quote.total_mxn ?? 0)))}</strong><p>Propuesta preliminar sujeta a validacion tecnica en sitio.</p></div><div style="text-align:right"><span class="label">Sistema recomendado</span><div class="value" style="color:#fff">${esc(quote.panel_count ?? result.panelCount ?? '-')} paneles</div><div style="font-size:13px;color:#d4e0e8;margin-top:5px">${esc(module.watts ?? '-')} W por panel</div></div></div>
    <div class="footer"><span>CDSE Solar - Los Mochis, Sinaloa</span><span>01 / 03</span></div></section>
